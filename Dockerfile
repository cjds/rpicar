...
RUN curl -s -N https://apt.llvm.org/llvm-snapshot.gpg.key|apt-key add -
RUN echo "deb [arch=amd64] https://storage.googleapis.com/bazel-apt stable jdk1.8" | tee /etc/apt/sources.list.d/bazel.list
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install --yes neovim bazel valgrind cppcheck wget clang-10 lldb-10 lld-10 libtinfo5 libgpiod-dev python3-pip
